...
async function verifySupabase() {
    ...
    try {
        ...
        // Test tables
        const tables = ['users', 'profiles', 'complaints', 'cases', 'evidence', 'notifications'];
...
async function verifyLiveBackend() {
    ...
    const url = 'https://epolix-backend.onrender.com/api/health';
    try {
        const res = await fetch(url);
        if (res.ok) {
            ...
            console.log(`❌ Backend check failed. Status: ${res.status} (Make sure your Render service name matches.)`);
        }
    } catch (e) {
